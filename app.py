...
st.set_page_config(page_title="Smart Visit Planner · V2.4.2", page_icon="⚡", layout="wide")
...
# UI
# ============================================================
st.title("Smart Visit Planner · V2.4.2")
...
def parse_pacto_direct(text):
    """Lectura directa de las filas de Evolución Pacto.
    Tolera que el OCR mezcle varias columnas de la ficha en una misma línea.
    """
    rows={}
    brand_patterns={
        "AVENE":r"\bAVENE\b",
        "DUCRAY":r"\bDUCRAY\b",
        "A-DERMA":r"\bA[\s-]?DERMA\b",
        "DEXERYL":r"\bDEXERYL\b",
        "KLORANE":r"\bKLORANE\b",
    }

    for raw in (text or "").splitlines():
        # Probamos cada aparición de marca dentro de la línea, porque el OCR puede
        # unir "Gamas prioritarias" y "Evolución Pacto" en la misma fila.
        for brand,pat in brand_patterns.items():
            for m in re.finditer(pat,raw,re.I):
                seg=raw[m.start():]
                vals=[parse_num(x) for x in re.findall(r"-?\d[\d\.,]*\s*%?",seg)]

                # Formato completo Evolución Pacto:
                # A-1 colectivo, actual colectivo, evol,
                # obj colectivo, falta colectivo,
                # A-1 marca, actual marca, evol marca, falta marca, ...
                if len(vals)>=9 and vals[0]>1000 and vals[1]>500 and vals[3]>1000:
                    rows[brand]={
                        "BrandKey":brand,
                        "Marca":BRAND_LABELS[brand],
                        "Obj. colectivo €":vals[3],
                        "Actual colectivo €":vals[1],
                        "Referencia marca €":vals[5],
                        "Actual marca €":vals[6],
                        "Falta marca €":vals[8],
                        "Fuente":"Evolución Pacto · lectura directa"
                    }
                    break
            if brand in rows:
                continue
...
def parse_ficha_v24(files):
    ...
    # V2.4.2: reconstrucción directa del pacto a partir del OCR real.
    direct=parse_pacto_direct(text)
    for b,r in direct.items():
        if "BrandKey" in adf.columns and (adf["BrandKey"]==b).any():
            idx=adf.index[adf["BrandKey"]==b][0]
            for k,v in r.items():
                adf.loc[idx,k]=v
        else:
            adf=pd.concat([adf,pd.DataFrame([r])],ignore_index=True)

    # Fallback financiero de AVÈNE SIN SOLAR.
    # La prioridad es la fila directa de Evolución Pacto; esta vía solo rescata
    # Avène si el OCR del pacto no ha sido suficiente.
    av_ref=av_cur=None
    if "BrandKey" in adf.columns and (adf["BrandKey"]=="AVENE").any():
        idx=adf.index[adf["BrandKey"]=="AVENE"][0]
        av_ref=parse_num(adf.loc[idx,"Referencia marca €"])
        av_cur=parse_num(adf.loc[idx,"Actual marca €"])

    if not av_ref:
        for raw in text.splitlines():
            compact=norm(raw).replace(" ","")
            if compact.startswith("AVENESINSOL"):
                vals=[parse_num(x) for x in re.findall(r"-?\d[\d\.,]*\s*%?",raw)]
                if len(vals)>=3:
                    av_ref=vals[0]
                    av_cur=vals[2]
                    break
    ...
    if av_ref and av_cur is not None and not (("BrandKey" in adf.columns) and (adf["BrandKey"]=="AVENE").any()):
        adf=pd.concat([adf,pd.DataFrame([{
            ...
            "Falta marca €":max(av_ref-av_cur,0),
            "Fuente":"Ficha 2026 · Avène validado"
        }])],ignore_index=True)
