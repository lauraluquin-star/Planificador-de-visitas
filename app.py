...
st.set_page_config(page_title="Smart Visit Planner · V2.2", page_icon="⚡", layout="wide")
...
def parse_ficha(files):
    ...
    main=adf[adf["BrandKey"].isin(["AVENE","DUCRAY","A-DERMA"])] if not adf.empty else pd.DataFrame()
...
def opportunity_and_focus(ficha,cats,current_month):
    ...
    if not agr.empty:
        for _,r in agr.sort_values("Falta marca €",ascending=False).iterrows():
            if r["BrandKey"] in ["AVENE","DUCRAY","A-DERMA"] and r["Falta marca €"]>0:
                sev="red" if r["Falta marca €"]>1000 else "amber"
                actions.append({"sev":sev,"title":f"RECUPERAR {r['Marca'].upper()}",
                                "text":f"Gap de marca: {euro(r['Falta marca €'])}. Priorizar pedido y palancas del ciclo."})
    # Only true opportunity if DCC itself appears 0/0. Never convert DCG into anticaída.
    if not cats.empty:
        for _,r in cats.iterrows():
            y=parse_num(r["YTD uds"]);t=parse_num(r["TAM12M uds"])
            if y==0 and t==0 and r["Gama"] not in ["AP Capilares","Epitheliale","Indispensables"]:
                actions.append({"sev":"blue","title":"OPORTUNIDAD REAL",
                                "text":f"{r['Marca']} · {r['Gama']}: 0 uds YTD y 0 uds TAM12M. Preguntar qué necesita para introducir la gama."})
            elif t>0:
                remaining=max(t-y,0)
                if remaining>=20:
                    actions.append({"sev":"amber","title":"ROTACIÓN / ESTACIONALIDAD",
                                    "text":f"{r['Marca']} · {r['Gama']}: {int(y)} uds YTD; {int(remaining)} uds corresponden al tramo restante del TAM12M."})
...
# UI
# ============================================================
st.title("Smart Visit Planner · V2.2")
...
with c[3]:
    ytd=parse_num(current.get("TOTAL_YTD",0));obj=parse_num(current.get("TOTAL_OBJ",0))
    ratio=ytd/obj if obj else 0
    sev=status_from_ratio(ratio,date.today().timetuple().tm_yday/365)
    st.markdown(f"<div class='card card-{sev}'><div class='kicker'>LOB · ritmo</div><div class='big'>{ratio:.0%}</div><div class='small'>{euro(ytd)} YTD · solo contraste</div></div>",unsafe_allow_html=True)
with c[4]:
...
if vcats.empty:
    ...
    v["Lectura"]=v.apply(lambda r:"OPORTUNIDAD" if r["YTD uds"]==0 and r["TAM12M uds"]==0 else
                                   ("REVISAR" if r["Resto TAM12M uds"]>=20 else "MANTENER"),axis=1)
