...
st.set_page_config(page_title="Smart Visit Planner · V2.4", page_icon="⚡", layout="wide")
...
# UI
# ============================================================
st.title("Smart Visit Planner · V2.4")
...
def parse_ficha_v24(files):
    ...
    # Lectura explícita de la fila financiera AVÈNE SIN SOLAR.
    # En las capturas reales aparece como: AVENESINSOL. 17.521 12.868 12.384 -4%
    av_ref=av_cur=None
    m=re.search(r"AVENE\s*SIN\s*SOL\.?\s+(\d[\d\.]*)\s+(\d[\d\.]*)\s+[«“]?\s*(\d[\d\.]*)\s+(-?\d+)%|AVENESINSOL\.?\s+(\d[\d\.]*)\s+(\d[\d\.]*)\s+[«“]?\s*(\d[\d\.]*)\s+(-?\d+)%",text,re.I)
    if m:
        vals=[x for x in m.groups() if x is not None]
        if len(vals)>=3:
            av_ref=parse_num(vals[0]); av_cur=parse_num(vals[2])

    # Segunda vía: la tabla financiera que ya extrae V2.3.
    if not av_ref:
